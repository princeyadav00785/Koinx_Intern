  ...
  useEffect(() => {
    const fetchTrendingCoins = async () => {
      try {
        ...
        // console.log(data)
        const top25TrendingCoins = data.coins.slice(10, 20);
        ...
        {trendingCoins.map((coin) => (
          <div key={coin.item.id} className="flex flex-col items-center p-4 border rounded-md shadow-md">
           ...
           <img
            ...
            <p className="text-green-500 font-semibold mt-8 ml-4">{parseFloat(coin.item.data.price_change_percentage_24h.usd).toFixed(2)}%</p>
           </div>
